import { Check, Clock, Loader2, MapPin, Monitor, Users, X } from 'lucide-react';
import { useState } from 'react';
import { toast } from 'sonner';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import type { Group } from '@/types';
import { SUBJECT_COLORS } from '@/utils/constant';
import { extractErrorMessage, formatDate, formatDateTime } from '@/utils/helpers';
      ...
      <CardContent className="p-4 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span
              className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${subjectColor(group.subject)}`}
            >
              {group.subject}
            </span>
          </div>
